...
export async function playAudio(
  ...
  if (options.player) {
    try {
      await execAsync(`${options.player} "${audioPath}"`);
      return;
    } catch (error) {
      throw new Error(
  ...
  switch (osPlatform) {
    ...
    case 'win32': // Windows
      // Try PowerShell with Windows Media Player (waits for completion)
      // Escape path for PowerShell: single quotes need doubling, use forward slashes
      const psPath = audioPath.replace(/'/g, "''").replace(/\\/g, '/');
    ...
    case 'linux':
      ...
      // We'll try them in sequence with proper error handling
      const linuxPlayers = [
        { cmd: 'mpg123', args: ['-q', audioPath] },
        { cmd: 'paplay', args: [audioPath] },
        { cmd: 'aplay', args: [audioPath] },
      ];
